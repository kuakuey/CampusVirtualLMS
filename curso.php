...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'guardar_asistencia' && $esPropietario) {
        if (!asegurar_tabla_asistencias()) {
            mensaje_flash('danger', 'No se pudo preparar la tabla de asistencias.');
            redirigir("curso.php?id=$id&pestaña=asistencia");
        }
        ...
        if (!is_array($estados) || !$estados) {
            ...
            redirigir("curso.php?id=$id&pestaña=asistencia&fecha=" . urlencode($fechaGuardar));
        }
...
$vistaAsistencia = $_GET['vista'] ?? 'tomar';
if (!in_array($vistaAsistencia, ['tomar', 'reportes'], true)) {
    $vistaAsistencia = 'tomar';
}
$fechaAsistencia = fecha_asistencia_valida($_GET['fecha'] ?? '') ?: date('Y-m-d');
$desdeAsistencia = fecha_asistencia_valida($_GET['desde'] ?? '') ?: date('Y-m-01');
$hastaAsistencia = fecha_asistencia_valida($_GET['hasta'] ?? '') ?: date('Y-m-d');
if ($desdeAsistencia > $hastaAsistencia) {
    $tmp = $desdeAsistencia;
    $desdeAsistencia = $hastaAsistencia;
    $hastaAsistencia = $tmp;
}

$estudiantesAsistencia = [];
$asistenciasFecha = [];
$yaRegistradaAsistencia = false;
$reporteEstudiantes = [];
$reporteFechas = [];
$detalleAsistenciaPropia = [];
$resumenAsistencia = [
    'sesiones' => 0,
    'presentes' => 0,
    'ausentes' => 0,
...
if ($pestaña === 'asistencia') {
    ...
    if ($esPropietario) {
        ...
        $asistenciasFecha = asistencias_por_fecha($id, $fechaAsistencia);
        $yaRegistradaAsistencia = count($asistenciasFecha) > 0;
        $reporteEstudiantes = reporte_asistencia_estudiantes($id, $desdeAsistencia, $hastaAsistencia);
        $reporteFechas = reporte_asistencia_fechas($id, $desdeAsistencia, $hastaAsistencia);
        $resumenAsistencia = resumen_desde_fechas($reporteFechas);
    } else {
        $detalleAsistenciaPropia = asistencias_estudiante_curso((int) $usuario['id'], $id);
        $resumenAsistencia = resumen_asistencias($detalleAsistenciaPropia);
    }
}
...
<div class="panel">
    <div class="panel-header">
        ...
        <a href="?id=<?= $id ?>&pestaña=asistencia" class="btn btn-sm btn-primary">
...
<?php elseif ($pestaña === 'asistencia'): ?>
<?php if ($esPropietario): ?>
<ul class="nav nav-pills gap-2 mb-4 flex-wrap">
    <li class="nav-item">
        <a class="nav-link <?= $vistaAsistencia === 'tomar' ? 'active' : '' ?>" href="?id=<?= $id ?>&pestaña=asistencia&vista=tomar&fecha=<?= urlencode($fechaAsistencia) ?>">
            <i class="bi bi-calendar-check me-1"></i> Tomar asistencia
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link <?= $vistaAsistencia === 'reportes' ? 'active' : '' ?>" href="?id=<?= $id ?>&pestaña=asistencia&vista=reportes">
            <i class="bi bi-bar-chart me-1"></i> Reportes
        </a>
    </li>
</ul>

<?php if ($vistaAsistencia !== 'reportes'): ?>
<div class="panel mb-4">
    ...
    <div class="panel-body">
        <form class="row g-3 align-items-end" method="get">
            ...
            <div class="col-md-6">
                ...
                <input type="date" name="fecha" id="fecha-asistencia" class="form-control" value="<?= escapar($fechaAsistencia) ?>" required>
            </div>
            <div class="col-md-3">
...
<?php else: ?>
    <form method="post" id="form-asistencia">
        ...
        <input type="hidden" name="fecha" value="<?= escapar($fechaAsistencia) ?>">
        <div class="panel">
            <div class="panel-header">
                <div>
                    <h2 class="mb-1">Lista del <?= formatear_fecha($fechaAsistencia) ?></h2>
                    <p class="small text-muted mb-0">
                        <?= count($estudiantesAsistencia) ?> estudiante(s)
                        <?php if ($yaRegistradaAsistencia): ?>
    ...
    <div class="panel-body">
        <form class="row g-3 align-items-end" method="get">
            ...
            <div class="col-md-4">
                ...
                <input type="date" name="desde" id="desde-asistencia" class="form-control" value="<?= escapar($desdeAsistencia) ?>">
            </div>
            <div class="col-md-4">
                <label class="form-label" for="hasta-asistencia">Hasta</label>
                <input type="date" name="hasta" id="hasta-asistencia" class="form-control" value="<?= escapar($hastaAsistencia) ?>">
            </div>
            <div class="col-md-4">
                <button class="btn btn-primary w-100" type="submit">
...
<div class="row g-3 mb-4">
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['sesiones'] ?></div>
    ...
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['promedio'] ?>%</div>
    ...
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['presentes'] ?></div>
    ...
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['ausentes'] ?></div>
                ...
                <tbody>
                    ...
                    <?php foreach ($reporteFechas as $fila): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                <a class="btn btn-sm btn-outline-primary" href="?id=<?= $id ?>&pestaña=asistencia&vista=tomar&fecha=<?= urlencode($fila['attendance_date']) ?>">
                                    Ver lista
                                </a>
                            </td>
                        </tr>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['tardes'] ?></div>
    ...
    <div class="col-md-3 col-6">
        <div class="stat-card">
            ...
            <div class="stat-value"><?= (int) $resumenAsistencia['justificados'] ?></div>
