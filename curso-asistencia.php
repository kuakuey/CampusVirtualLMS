...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'guardar_asistencia' && $esPropietario) {
        ...
        if (!is_array($estados) || !$estados) {
            ...
            redirigir("curso-asistencia.php?id=$id&fecha=" . urlencode($fechaGuardar));
        }
...
<ul class="nav nav-pills gap-2 mb-4 flex-wrap">
    <li class="nav-item">
        <a class="nav-link <?= $vista === 'tomar' ? 'active' : '' ?>" href="?id=<?= $id ?>&vista=tomar&fecha=<?= urlencode($fecha) ?>">
            <i class="bi bi-calendar-check me-1"></i> Tomar asistencia
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link <?= $vista === 'reportes' ? 'active' : '' ?>" href="?id=<?= $id ?>&vista=reportes">
                ...
                <tbody>
                    ...
                    <?php foreach ($reporteFechas as $fila): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                <a class="btn btn-sm btn-outline-primary" href="?id=<?= $id ?>&vista=tomar&fecha=<?= urlencode($fila['attendance_date']) ?>">
                                    Ver lista
                                </a>
                            </td>
                        </tr>
